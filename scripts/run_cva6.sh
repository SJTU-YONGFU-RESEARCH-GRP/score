...
info() { echo -e "${BLUE}[run_cva6]${NC} $*"; }
ok() { echo -e "${GREEN}[run_cva6]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_cva6]${NC} $*"; }
err() { echo -e "${RED}[run_cva6]${NC} $*" >&2; }
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $CVA6_SUBMODULE"
    git submodule update --init --recursive "$CVA6_SUBMODULE"
    ok "Submodules ready."
else
