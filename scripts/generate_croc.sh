...
SOURCE_CROC_DIR="$PROJECT_ROOT/tools/croc"
CROC_DIR=""
...
init_script_logging generate_croc
...
if [[ ! -d "$SOURCE_CROC_DIR" ]]; then
    err "Missing $SOURCE_CROC_DIR — run: git submodule update --init --recursive tools/croc"
    exit 1
fi
...
get_commit_id() {
    if git -C "$SOURCE_CROC_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        git -C "$SOURCE_CROC_DIR" rev-parse --short=8 HEAD 2>/dev/null || echo "unknown"
    else
        echo "unknown"
    fi
}
...
prepare_dataset_workdir() {
    info "Staging Croc sources into dataset workdir: $CROC_DIR"
    mkdir -p "$CROC_DIR"
    rsync -a --delete \
        --exclude '.git/' \
        "$SOURCE_CROC_DIR/" "$CROC_DIR/"
}

exec > >(tee -a "$SESSION_LOG") 2>&1

info "SOURCE_CROC_DIR: $SOURCE_CROC_DIR"
info "WORK_DIR: $CROC_DIR"
...
SUMMARY="$DATASET_DIR/croc_summary.txt"
{
    ...
    echo "Source repo: $SOURCE_CROC_DIR"
    echo "Dataset workdir: $CROC_DIR"
    echo "Git commit (short): $CROC_COMMIT_ID"
    echo "Git commit (full): $(git -C "$SOURCE_CROC_DIR" rev-parse HEAD 2>/dev/null || echo unknown)"
