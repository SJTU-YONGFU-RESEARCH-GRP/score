...
info() { echo -e "${BLUE}[generate_safety_island]${NC} $*"; }
ok() { echo -e "${GREEN}[generate_safety_island]${NC} $*"; }
warn() { echo -e "${YELLOW}[generate_safety_island]${NC} $*"; }
err() { echo -e "${RED}[generate_safety_island]${NC} $*" >&2; }
...
{
    echo "safety_island SCORE snapshot"
