...
log() {
    ...
    echo -e "${BLUE}[$timestamp]${NC} $message" | tee -a "$GENERATION_LOG"
}

error() {
    local message="$1"
    local timestamp="$(date +'%Y-%m-%d %H:%M:%S')"
    echo -e "${RED}[ERROR $timestamp]${NC} $message" | tee -a "$GENERATION_LOG" >&2
}

success() {
    local message="$1"
    local timestamp="$(date +'%Y-%m-%d %H:%M:%S')"
    echo -e "${GREEN}[SUCCESS $timestamp]${NC} $message" | tee -a "$GENERATION_LOG"
}

warning() {
    local message="$1"
    local timestamp="$(date +'%Y-%m-%d %H:%M:%S')"
    echo -e "${YELLOW}[WARNING $timestamp]${NC} $message" | tee -a "$GENERATION_LOG"
}

info() {
    local message="$1"
    local timestamp="$(date +'%Y-%m-%d %H:%M:%S')"
    echo -e "${CYAN}[INFO $timestamp]${NC} $message" | tee -a "$GENERATION_LOG"
}
...
debug() {
    if [[ "$VERBOSE" == "true" ]]; then
        local message="$1"
        local timestamp="$(date +'%Y-%m-%d %H:%M:%S')"
        echo -e "${PURPLE}[DEBUG $timestamp]${NC} $message" | tee -a "$GENERATION_LOG"
    fi
}
