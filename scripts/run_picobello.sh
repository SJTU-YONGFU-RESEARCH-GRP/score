#!/usr/bin/env bash
#
# End-to-end picobello flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_picobello.sh
#   ./scripts/run_picobello.sh --no-system-deps
#   ./scripts/run_picobello.sh -- --skip-checkout
#
...
PICOBELLO_SUBMODULE="tools/picobello"
INSTALL_SCRIPT="$SCRIPT_DIR/install_picobello.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_picobello.sh"
...
info() { echo -e "${BLUE}[run_picobello]${NC} $*"; }
ok() { echo -e "${GREEN}[run_picobello]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_picobello]${NC} $*"; }
err() { echo -e "${RED}[run_picobello]${NC} $*" >&2; }

show_help() {
    cat << EOF
Usage: $0 [OPTIONS] [-- EXTRA_GENERATE_ARGS...]

Run the full SCORE picobello pipeline from the repository root:
  1. git submodule update --init --recursive ${PICOBELLO_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages)
  3. ${GENERATE_SCRIPT} (bender checkout + snapshot under datasets/picobello/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_picobello.sh
...
validate_root() {
    if [[ ! -f "$PROJECT_ROOT/.gitmodules" ]] || [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_picobello.sh"
        exit 1
    fi
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $PICOBELLO_SUBMODULE"
    git submodule update --init --recursive "$PICOBELLO_SUBMODULE"
    ok "Submodules ready."
else
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_picobello.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    if [[ ${#GENERATE_EXTRA[@]} -gt 0 ]]; then
        info "Step 3/3: generate_picobello.sh (extra: ${GENERATE_EXTRA[*]})"
    else
        info "Step 3/3: generate_picobello.sh (bender checkout + snapshot)"
    fi
...
ok "run_picobello.sh completed successfully."
