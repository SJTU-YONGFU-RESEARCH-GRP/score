#!/usr/bin/env bash
#
# End-to-end cheshire flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_cheshire.sh
#   ./scripts/run_cheshire.sh --no-system-deps
#   ./scripts/run_cheshire.sh --no-verilator
#   ./scripts/run_cheshire.sh --no-full-sim
#   ./scripts/run_cheshire.sh -- --skip-checkout
#
...
CHESHIRE_SUBMODULE="tools/cheshire"
INSTALL_SCRIPT="$SCRIPT_DIR/install_cheshire.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_cheshire.sh"
...
info() { echo -e "${BLUE}[run_cheshire]${NC} $*"; }
ok() { echo -e "${GREEN}[run_cheshire]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_cheshire]${NC} $*"; }
err() { echo -e "${RED}[run_cheshire]${NC} $*" >&2; }
...
Run the full SCORE cheshire pipeline from the repository root:
  1. git submodule update --init --recursive ${CHESHIRE_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages; Verilator + C++ by default)
  3. ${GENERATE_SCRIPT} (bender checkout, verification + snapshot under datasets/cheshire/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_cheshire.sh
...
validate_root() {
    if [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_cheshire.sh"
        exit 1
    fi
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_cheshire.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    ...
    if [[ ${#GEN_ARGS[@]} -gt 0 ]]; then
        info "Step 3/3: generate_cheshire.sh (extra: ${GEN_ARGS[*]})"
    else
        info "Step 3/3: generate_cheshire.sh (bender checkout + verification + snapshot)"
    fi
...
ok "run_cheshire.sh completed successfully."
TOOL_DIR="$PROJECT_ROOT/tools/cheshire"
CHESHIRE_HEAD_SHORT="$(get_commit_id "$TOOL_DIR")"
info "Dataset path for current tools/cheshire HEAD: $PROJECT_ROOT/datasets/cheshire/$CHESHIRE_HEAD_SHORT"
info "Summary: $PROJECT_ROOT/datasets/cheshire/$CHESHIRE_HEAD_SHORT/cheshire_summary.txt"
