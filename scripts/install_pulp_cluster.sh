#!/usr/bin/env bash
#
# Install host tools for pulp-platform/pulp_cluster under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/pulp-cluster; generation runs bender checkout and snapshots sources.
#
# Usage (from repository root):
#   ./scripts/install_pulp_cluster.sh
#   ./scripts/install_pulp_cluster.sh --no-system-deps
#   ./scripts/install_pulp_cluster.sh --skip-bender
#
...
Install dependencies for tools/pulp-cluster (Bender + common OS packages).
...
ok "install_pulp_cluster.sh finished."
info "Next: source scripts/pulp_cluster_env.sh"
info "Then:  ./scripts/generate_pulp_cluster.sh"
