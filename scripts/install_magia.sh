#!/usr/bin/env bash
#
# Install host tools for pulp-platform/MAGIA under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/magia; generation runs bender checkout and snapshots sources.
#
...
info() { echo -e "${BLUE}[install_magia]${NC} $*"; }
ok() { echo -e "${GREEN}[install_magia]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_magia]${NC} $*"; }
err() { echo -e "${RED}[install_magia]${NC} $*" >&2; }
...
Install dependencies for tools/magia (Bender + common OS packages).
...
ok "install_magia.sh finished."
info "Next: source scripts/magia_env.sh"
info "Then:  ./scripts/generate_magia.sh"
