#!/usr/bin/env bash
#
# Install host tools for pulp-platform/picobello under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/picobello; generation runs bender checkout and snapshots sources.
# Checkout can be large (many Bender IPs + cva6 submodules); allow time and disk.
#
# Usage (from repository root):
#   ./scripts/install_picobello.sh
#   ./scripts/install_picobello.sh --no-system-deps
#   ./scripts/install_picobello.sh --skip-bender
#
...
info() { echo -e "${BLUE}[install_picobello]${NC} $*"; }
ok() { echo -e "${GREEN}[install_picobello]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_picobello]${NC} $*"; }
err() { echo -e "${RED}[install_picobello]${NC} $*" >&2; }
...
Install dependencies for tools/picobello (Bender + common OS packages).
...
After installation:
  source scripts/picobello_env.sh
...
ok "install_picobello.sh finished."
info "Next: source scripts/picobello_env.sh"
info "Then:  ./scripts/generate_picobello.sh"
