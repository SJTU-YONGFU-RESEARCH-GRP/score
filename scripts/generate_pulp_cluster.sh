...
init_script_logging generate_pulp_cluster
...
RTL_DIR="$DATASET_DIR/rtl_designs/pulp_cluster_snapshot"
...
config_name: pulp_cluster_snapshot
generation_date_utc: $(date -u '+%Y-%m-%d %H:%M:%S')
git_commit_short: $PULP_CLUSTER_COMMIT_ID
source_snapshot: $BUNDLE_DIR
EOF

cat > "$RTL_DIR/pulp_cluster_smoke_top.v" << 'EOF'
module pulp_cluster_smoke_top (
    input wire clk,
    input wire rst_n,
    output wire ready
);
...
cat > "$RTL_DIR/pulp_cluster_smoke_tb.sv" << 'EOF'
`timescale 1ns/1ps
module pulp_cluster_smoke_tb;
    reg clk = 1'b0;
    reg rst_n = 1'b0;
    wire ready;
...
if command_exists verilator; then
    if verilator --lint-only -Wall -Wno-fatal --timing \
        --top-module pulp_cluster_smoke_tb \
        "$RTL_DIR/pulp_cluster_smoke_top.v" "$RTL_DIR/pulp_cluster_smoke_tb.sv" >"$VERILATOR_LOG" 2>&1; then
...
{
    echo "pulp_cluster SCORE snapshot (pulp-platform/pulp_cluster)"
...
{
    echo "pulp_cluster verification summary"
    echo "Generated (UTC): $(date -u '+%Y-%m-%d %H:%M:%S')"
