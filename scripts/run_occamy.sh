#!/usr/bin/env bash
#
# End-to-end occamy flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_occamy.sh
#   ./scripts/run_occamy.sh --no-system-deps
#   ./scripts/run_occamy.sh -- --skip-checkout
#
...
OCCAMY_SUBMODULE="tools/occamy"
INSTALL_SCRIPT="$SCRIPT_DIR/install_occamy.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_occamy.sh"
...
info() { echo -e "${BLUE}[run_occamy]${NC} $*"; }
ok() { echo -e "${GREEN}[run_occamy]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_occamy]${NC} $*"; }
err() { echo -e "${RED}[run_occamy]${NC} $*" >&2; }

show_help() {
    cat << EOF
Usage: $0 [OPTIONS] [-- EXTRA_GENERATE_ARGS...]

Run the full SCORE occamy pipeline from the repository root:
  1. git submodule update --init --recursive ${OCCAMY_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages)
  3. ${GENERATE_SCRIPT} (bender checkout + snapshot under datasets/occamy/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_occamy.sh
...
validate_root() {
    if [[ ! -f "$PROJECT_ROOT/.gitmodules" ]] || [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_occamy.sh"
        exit 1
    fi
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_occamy.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    if [[ ${#GENERATE_EXTRA[@]} -gt 0 ]]; then
        info "Step 3/3: generate_occamy.sh (extra: ${GENERATE_EXTRA[*]})"
    else
        info "Step 3/3: generate_occamy.sh (bender checkout + snapshot)"
    fi
...
ok "run_occamy.sh completed successfully."
