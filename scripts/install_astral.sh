#!/usr/bin/env bash
#
# Install host tools for pulp-platform/astral under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/astral; generation runs bender checkout and snapshots sources.
# Note: generate_astral.sh may clone large nested trees (Cheshire-related IPs, CVA6); allow time and disk.
#
# Usage (from repository root):
#   ./scripts/install_astral.sh
#   ./scripts/install_astral.sh --no-system-deps
#   ./scripts/install_astral.sh --skip-bender
#   ./scripts/install_astral.sh --no-verilator   (skip Verilator + C++ build deps; generate_astral.sh will skip Verilator steps)
#
...
info() { echo -e "${BLUE}[install_astral]${NC} $*"; }
ok() { echo -e "${GREEN}[install_astral]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_astral]${NC} $*"; }
err() { echo -e "${RED}[install_astral]${NC} $*" >&2; }
...
Install dependencies for tools/astral (Bender + common OS packages).
...
Options:
  ...
  --no-verilator       Do not install Verilator or C++ toolchain packages (for generate_astral.sh --skip-verilator)
...
install_system_packages() {
    ...
    case "$os" in
        ...
        macos)
            ...
            if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
                brew install verilator || warn "Verilator brew install failed; install manually for generate_astral.sh"
            fi
            ;;
        *)
...
if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
    if command_exists verilator; then
        ...
        warn "verilator not on PATH after package step (generate_astral.sh will skip Verilator unless you install it)"
    fi
    if command_exists g++ || command_exists c++; then
        ok "C++ compiler available for Verilator --binary (generate_astral.sh)"
    else
        warn "No g++/c++ on PATH; Verilator binary build in generate_astral.sh may fail"
    fi
else
    info "Skipped Verilator/C++ package install (--no-verilator)"
fi

ok "install_astral.sh finished."
info "Next: source scripts/astral_env.sh"
info "Then:  ./scripts/generate_astral.sh"
