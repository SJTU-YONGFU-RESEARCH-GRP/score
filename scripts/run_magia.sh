...
info() { echo -e "${BLUE}[run_magia]${NC} $*"; }
ok() { echo -e "${GREEN}[run_magia]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_magia]${NC} $*"; }
err() { echo -e "${RED}[run_magia]${NC} $*" >&2; }
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $MAGIA_SUBMODULE"
    git submodule update --init --recursive "$MAGIA_SUBMODULE"
    ok "Submodules ready."
else
