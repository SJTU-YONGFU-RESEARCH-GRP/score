#!/usr/bin/env bash
#
# Install host tools for pulp-platform/pulp_soc under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/pulp-soc; generation writes Bender.local (gpio pin) then bender checkout.
#
# Usage (from repository root):
#   ./scripts/install_pulp_soc.sh
#   ./scripts/install_pulp_soc.sh --no-system-deps
#   ./scripts/install_pulp_soc.sh --skip-bender
#
...
info() { echo -e "${BLUE}[install_pulp_soc]${NC} $*"; }
ok() { echo -e "${GREEN}[install_pulp_soc]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_pulp_soc]${NC} $*"; }
err() { echo -e "${RED}[install_pulp_soc]${NC} $*" >&2; }
...
Install dependencies for tools/pulp-soc (Bender + common OS packages).
...
ok "install_pulp_soc.sh finished."
info "Next: source scripts/pulp_soc_env.sh"
info "Then:  ./scripts/generate_pulp_soc.sh"
