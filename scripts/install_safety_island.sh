#!/usr/bin/env bash
#
# Install host tools for pulp-platform/safety_island under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/safety-island; generation runs bender checkout and snapshots sources.
#
...
info() { echo -e "${BLUE}[install_safety_island]${NC} $*"; }
ok() { echo -e "${GREEN}[install_safety_island]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_safety_island]${NC} $*"; }
err() { echo -e "${RED}[install_safety_island]${NC} $*" >&2; }
...
Install dependencies for tools/safety-island (Bender + common OS packages).
...
ok "install_safety_island.sh finished."
info "Next: source scripts/safety_island_env.sh"
info "Then:  ./scripts/generate_safety_island.sh"
