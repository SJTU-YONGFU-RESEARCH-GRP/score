#!/usr/bin/env bash
#
# Install host tools for pulp-platform/auteur under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/auteur; generation runs bender checkout, default Verilator verification, snapshot.
#
# Usage (from repository root):
#   ./scripts/install_auteur.sh
#   ./scripts/install_auteur.sh --no-system-deps
#   ./scripts/install_auteur.sh --skip-bender
#   ./scripts/install_auteur.sh --no-verilator   (skip Verilator + C++; use generate --skip-verilator)
#
...
info() { echo -e "${BLUE}[install_auteur]${NC} $*"; }
ok() { echo -e "${GREEN}[install_auteur]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_auteur]${NC} $*"; }
err() { echo -e "${RED}[install_auteur]${NC} $*" >&2; }
...
Install dependencies for tools/auteur (Bender + common OS packages).
...
Options:
  ...
  --no-verilator       Do not install Verilator or C++ toolchain packages (for generate_auteur.sh --skip-verilator)
...
install_system_packages() {
    ...
    case "$os" in
        ...
        macos)
            ...
            if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
                brew install verilator || warn "Verilator brew install failed; install manually for generate_auteur.sh"
            fi
            ;;
        *)
...
if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
    if command_exists verilator; then
        ...
        warn "verilator not on PATH after package step (generate_auteur.sh will need it or use --skip-verilator)"
    fi
    if command_exists g++ || command_exists c++; then
        ok "C++ compiler available for Verilator --binary (generate_auteur.sh)"
    else
        warn "No g++/c++ on PATH; Verilator binary build in generate_auteur.sh may fail"
    fi
else
    info "Skipped Verilator/C++ package install (--no-verilator)"
fi

ok "install_auteur.sh finished."
info "Next: source scripts/auteur_env.sh"
info "Then:  ./scripts/generate_auteur.sh"
