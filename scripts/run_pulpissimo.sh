#!/usr/bin/env bash
#
# End-to-end PULPissimo flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_pulpissimo.sh
#   ./scripts/run_pulpissimo.sh --no-system-deps
#   ./scripts/run_pulpissimo.sh -- --skip-checkout
#
...
PULPISSIMO_SUBMODULE="tools/pulpissimo"
INSTALL_SCRIPT="$SCRIPT_DIR/install_pulpissimo.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_pulpissimo.sh"
...
info() { echo -e "${BLUE}[run_pulpissimo]${NC} $*"; }
ok() { echo -e "${GREEN}[run_pulpissimo]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_pulpissimo]${NC} $*"; }
err() { echo -e "${RED}[run_pulpissimo]${NC} $*" >&2; }

show_help() {
    cat << EOF
Usage: $0 [OPTIONS] [-- EXTRA_GENERATE_ARGS...]

Run the full SCORE pulpissimo pipeline from the repository root:
  1. git submodule update --init --recursive ${PULPISSIMO_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages)
  3. ${GENERATE_SCRIPT} (bender checkout + snapshot under datasets/pulpissimo/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_pulpissimo.sh
...
validate_root() {
    if [[ ! -f "$PROJECT_ROOT/.gitmodules" ]] || [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_pulpissimo.sh"
        exit 1
    fi
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $PULPISSIMO_SUBMODULE"
    git submodule update --init --recursive "$PULPISSIMO_SUBMODULE"
    ok "Submodules ready."
else
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_pulpissimo.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    if [[ ${#GENERATE_EXTRA[@]} -gt 0 ]]; then
        info "Step 3/3: generate_pulpissimo.sh (extra: ${GENERATE_EXTRA[*]})"
    else
        info "Step 3/3: generate_pulpissimo.sh (bender checkout + snapshot)"
    fi
...
ok "run_pulpissimo.sh completed successfully."
