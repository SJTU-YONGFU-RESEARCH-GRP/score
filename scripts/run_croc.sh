#!/usr/bin/env bash
#
# End-to-end croc flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_croc.sh
#   ./scripts/run_croc.sh --no-system-deps
#   ./scripts/run_croc.sh --no-verilator
#   ./scripts/run_croc.sh --no-full-sim
#   ./scripts/run_croc.sh -- --skip-checkout
#
...
CROC_SUBMODULE="tools/croc"
INSTALL_SCRIPT="$SCRIPT_DIR/install_croc.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_croc.sh"
...
Run the full SCORE croc pipeline from the repository root:
  1. git submodule update --init --recursive ${CROC_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages; Verilator + C++ by default)
  3. ${GENERATE_SCRIPT} (bender checkout, verification + snapshot under datasets/croc/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_croc.sh
...
validate_root() {
    if [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_croc.sh"
        exit 1
    fi
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $CROC_SUBMODULE"
    git submodule update --init --recursive "$CROC_SUBMODULE"
    ok "Submodules ready."
else
...
if [[ "$SKIP_GENERATE" != true ]]; then
    ...
    if [[ ${#GEN_ARGS[@]} -gt 0 ]]; then
        info "Step 3/3: generate_croc.sh (extra: ${GEN_ARGS[*]})"
    else
        info "Step 3/3: generate_croc.sh (bender checkout + verification + snapshot)"
    fi
...
ok "run_croc.sh completed successfully."
TOOL_DIR="$PROJECT_ROOT/tools/croc"
CROC_HEAD_SHORT="$(get_commit_id "$TOOL_DIR")"
info "Dataset path for current tools/croc HEAD: $PROJECT_ROOT/datasets/croc/$CROC_HEAD_SHORT"
info "Summary: $PROJECT_ROOT/datasets/croc/$CROC_HEAD_SHORT/croc_summary.txt"
