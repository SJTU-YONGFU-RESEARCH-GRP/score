#!/usr/bin/env bash
#
# End-to-end pulp_cluster flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_pulp_cluster.sh
#   ./scripts/run_pulp_cluster.sh --no-system-deps
#   ./scripts/run_pulp_cluster.sh -- --skip-checkout
#
...
PULP_CLUSTER_SUBMODULE="tools/pulp-cluster"
INSTALL_SCRIPT="$SCRIPT_DIR/install_pulp_cluster.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_pulp_cluster.sh"
...
info() { echo -e "${BLUE}[run_pulp_cluster]${NC} $*"; }
ok() { echo -e "${GREEN}[run_pulp_cluster]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_pulp_cluster]${NC} $*"; }
err() { echo -e "${RED}[run_pulp_cluster]${NC} $*" >&2; }

show_help() {
    cat << EOF
Usage: $0 [OPTIONS] [-- EXTRA_GENERATE_ARGS...]

Run the full SCORE pulp_cluster pipeline from the repository root:
  1. git submodule update --init --recursive ${PULP_CLUSTER_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages)
  3. ${GENERATE_SCRIPT} (bender checkout + snapshot under datasets/pulp_cluster/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_pulp_cluster.sh
...
validate_root() {
    if [[ ! -f "$PROJECT_ROOT/.gitmodules" ]] || [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_pulp_cluster.sh"
        exit 1
    fi
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $PULP_CLUSTER_SUBMODULE"
    git submodule update --init --recursive "$PULP_CLUSTER_SUBMODULE"
    ok "Submodules ready."
else
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_pulp_cluster.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    if [[ ${#GENERATE_EXTRA[@]} -gt 0 ]]; then
        info "Step 3/3: generate_pulp_cluster.sh (extra: ${GENERATE_EXTRA[*]})"
    else
        info "Step 3/3: generate_pulp_cluster.sh (bender checkout + snapshot)"
    fi
...
ok "run_pulp_cluster.sh completed successfully."
