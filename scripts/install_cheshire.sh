#!/usr/bin/env bash
#
# Install host tools for pulp-platform/cheshire under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/cheshire; generation runs bender checkout, default Verilator verification, snapshot.
#
...
info() { echo -e "${BLUE}[install_cheshire]${NC} $*"; }
ok() { echo -e "${GREEN}[install_cheshire]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_cheshire]${NC} $*"; }
err() { echo -e "${RED}[install_cheshire]${NC} $*" >&2; }
...
Install dependencies for tools/cheshire (Bender + common OS packages).
...
Options:
  ...
  --no-verilator       Do not install Verilator or C++ toolchain (use generate_cheshire.sh --skip-verilator)
...
install_system_packages() {
    ...
    case "$os" in
        ...
        macos)
            ...
            if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
                brew install verilator || warn "Verilator brew install failed; install manually for generate_cheshire.sh"
            fi
            ;;
        *)
...
if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
    if command_exists verilator; then
        ...
        warn "verilator not on PATH after package step (generate_cheshire.sh will need it or use --skip-verilator)"
    fi
    if command_exists g++ || command_exists c++; then
        ok "C++ compiler available for Verilator (generate_cheshire.sh)"
    else
        warn "No g++/c++ on PATH; Verilator steps in generate_cheshire.sh may fail"
    fi
else
    info "Skipped Verilator/C++ package install (--no-verilator)"
fi

ok "install_cheshire.sh finished."
info "Next: source scripts/cheshire_env.sh"
info "Then:  ./scripts/generate_cheshire.sh"
