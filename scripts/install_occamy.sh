#!/usr/bin/env bash
#
# Install host tools for pulp-platform/occamy under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/occamy; generation runs bender checkout and snapshots sources.
#
# Usage (from repository root):
#   ./scripts/install_occamy.sh
#   ./scripts/install_occamy.sh --no-system-deps
#   ./scripts/install_occamy.sh --skip-bender
#
...
info() { echo -e "${BLUE}[install_occamy]${NC} $*"; }
ok() { echo -e "${GREEN}[install_occamy]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_occamy]${NC} $*"; }
err() { echo -e "${RED}[install_occamy]${NC} $*" >&2; }
...
Install dependencies for tools/occamy (Bender + common OS packages).
...
ok "install_occamy.sh finished."
info "Next: source scripts/occamy_env.sh"
info "Then:  ./scripts/generate_occamy.sh"
