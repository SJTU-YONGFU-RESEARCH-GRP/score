#!/usr/bin/env bash
#
# End-to-end pulp_soc flow for SCORE: submodule → install → generate (Bender checkout, snapshot).
#
# Usage (from repository root, i.e. the directory containing tools/, scripts/, datasets/):
#   ./scripts/run_pulp_soc.sh
#   ./scripts/run_pulp_soc.sh --no-system-deps
#   ./scripts/run_pulp_soc.sh -- --skip-checkout
#
...
PULP_SOC_SUBMODULE="tools/pulp-soc"
INSTALL_SCRIPT="$SCRIPT_DIR/install_pulp_soc.sh"
GENERATE_SCRIPT="$SCRIPT_DIR/generate_pulp_soc.sh"
...
info() { echo -e "${BLUE}[run_pulp_soc]${NC} $*"; }
ok() { echo -e "${GREEN}[run_pulp_soc]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_pulp_soc]${NC} $*"; }
err() { echo -e "${RED}[run_pulp_soc]${NC} $*" >&2; }

show_help() {
    cat << EOF
Usage: $0 [OPTIONS] [-- EXTRA_GENERATE_ARGS...]

Run the full SCORE pulp_soc pipeline from the repository root:
  1. git submodule update --init --recursive ${PULP_SOC_SUBMODULE}
  2. ${INSTALL_SCRIPT} (Bender + OS packages)
  3. ${GENERATE_SCRIPT} (bender checkout + snapshot under datasets/pulp_soc/...)
...
Options:
  ...
  --                   All following arguments are passed only to generate_pulp_soc.sh
...
validate_root() {
    if [[ ! -f "$PROJECT_ROOT/.gitmodules" ]] || [[ ! -d "$PROJECT_ROOT/scripts" ]]; then
        err "Run this script from the SCORE repo: cd /path/to/score && ./scripts/run_pulp_soc.sh"
        exit 1
    fi
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $PULP_SOC_SUBMODULE"
    git submodule update --init --recursive "$PULP_SOC_SUBMODULE"
    ok "Submodules ready."
else
...
if [[ "$SKIP_INSTALL" != true ]]; then
    info "Step 2/3: install_pulp_soc.sh"
    INSTALL_ARGS=()
...
if [[ "$SKIP_GENERATE" != true ]]; then
    if [[ ${#GENERATE_EXTRA[@]} -gt 0 ]]; then
        info "Step 3/3: generate_pulp_soc.sh (extra: ${GENERATE_EXTRA[*]})"
    else
        info "Step 3/3: generate_pulp_soc.sh (bender checkout + snapshot)"
    fi
...
ok "run_pulp_soc.sh completed successfully."
