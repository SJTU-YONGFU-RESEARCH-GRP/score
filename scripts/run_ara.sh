...
info() { echo -e "${BLUE}[run_ara]${NC} $*"; }
ok() { echo -e "${GREEN}[run_ara]${NC} $*"; }
warn() { echo -e "${YELLOW}[run_ara]${NC} $*"; }
err() { echo -e "${RED}[run_ara]${NC} $*" >&2; }
...
if [[ "$SKIP_SUBMODULE" != true ]]; then
    info "Step 1/3: git submodule update --init --recursive $ARA_SUBMODULE"
    git submodule update --init --recursive "$ARA_SUBMODULE"
    ok "Submodules ready."
else
