#!/usr/bin/env bash
#
# Install host tools for pulp-platform/chimera under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/chimera; generation runs bender checkout, default Verilator verification, snapshot.
# Chimera may clone large Cheshire/cva6-sdk trees; allow time, disk, and network.
#
...
info() { echo -e "${BLUE}[install_chimera]${NC} $*"; }
ok() { echo -e "${GREEN}[install_chimera]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_chimera]${NC} $*"; }
err() { echo -e "${RED}[install_chimera]${NC} $*" >&2; }
...
Install dependencies for tools/chimera (Bender + common OS packages).
...
Options:
  ...
  --no-verilator       Do not install Verilator or C++ toolchain (use generate_chimera.sh --skip-verilator)
...
install_system_packages() {
    ...
    case "$os" in
        ...
        macos)
            ...
            if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
                brew install verilator || warn "Verilator brew install failed; install manually for generate_chimera.sh"
            fi
            ;;
        *)
...
if [[ "$INSTALL_VERILATOR_AND_CXX" == true ]]; then
    if command_exists verilator; then
        ...
        warn "verilator not on PATH after package step (generate_chimera.sh will need it or use --skip-verilator)"
    fi
    if command_exists g++ || command_exists c++; then
        ok "C++ compiler available for Verilator (generate_chimera.sh)"
    else
        warn "No g++/c++ on PATH; Verilator steps in generate_chimera.sh may fail"
    fi
else
    info "Skipped Verilator/C++ package install (--no-verilator)"
fi

ok "install_chimera.sh finished."
info "Next: source scripts/chimera_env.sh"
info "Then:  ./scripts/generate_chimera.sh"
