...
info() { echo -e "${BLUE}[generate_snitch_cluster]${NC} $*"; }
ok() { echo -e "${GREEN}[generate_snitch_cluster]${NC} $*"; }
warn() { echo -e "${YELLOW}[generate_snitch_cluster]${NC} $*"; }
err() { echo -e "${RED}[generate_snitch_cluster]${NC} $*" >&2; }
...
{
    echo "snitch_cluster SCORE snapshot"
