#!/usr/bin/env bash
#
# Install host tools for pulp-platform/cheshire-ihp130-o under SCORE (Bender, git, Python 3, rsync).
# RTL lives in tools/cheshire-ihp130-o; generation runs bender checkout and snapshots sources.
# Note: generate_cheshire_ihp130_o.sh may clone large nested trees (Cheshire / SDK); allow time and disk.
#
...
info() { echo -e "${BLUE}[install_cheshire_ihp130_o]${NC} $*"; }
ok() { echo -e "${GREEN}[install_cheshire_ihp130_o]${NC} $*"; }
warn() { echo -e "${YELLOW}[install_cheshire_ihp130_o]${NC} $*"; }
err() { echo -e "${RED}[install_cheshire_ihp130_o]${NC} $*" >&2; }
...
Install dependencies for tools/cheshire-ihp130-o (Bender + common OS packages).
...
ok "install_cheshire_ihp130_o.sh finished."
info "Next: source scripts/cheshire_ihp130_o_env.sh"
info "Then:  ./scripts/generate_cheshire_ihp130_o.sh"
