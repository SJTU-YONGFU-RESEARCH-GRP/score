#!/usr/bin/env bash
#
# Snapshot pulp-platform/cheshire RTL after Bender resolves Bender.lock (checkout).
# Output: datasets/cheshire/<short_sha>/source_snapshot/ (local SV + .bender checkouts).
#
# Verification (default, before rsync) → datasets/cheshire/<sha>/verification/:
...
# Full Cheshire SoC flist-plus is not required for this smoke path; it validates Bender + Verilator.
#
...
#   ./scripts/generate_cheshire.sh --bender-update
#   ./scripts/generate_cheshire.sh --no-verify
...
Environment:
  CHESHIRE_VERILATOR_JOBS    Parallel jobs for verilator (default: ${DEFAULT_VERILATOR_JOBS})
  CHESHIRE_FULL_SIM          If 0/false/no/off, same as --no-full-sim (after CLI flags)

Requires: git, bender on PATH, rsync (unless --verify-only); verilator + C++ for Verilator steps.
See https://github.com/pulp-platform/cheshire
EOF
}
...
if [[ -f "$PROJECT_ROOT/scripts/cheshire_env.sh" ]]; then
    # shellcheck source=/dev/null
    source "$PROJECT_ROOT/scripts/cheshire_env.sh"
fi
...
if ! command_exists bender; then
    err "bender not found on PATH. Run ./scripts/install_cheshire.sh"
    exit 1
fi
...
verify_cheshire_bender_lock_paths() {
    local lock="$CHESHIRE_DIR/Bender.lock" line pkg out missing
    missing=0
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        pkg="${BASH_REMATCH[1]}"
        if ( cd "$CHESHIRE_DIR" && bender path "$pkg" >/dev/null 2>&1 ); then
            echo "OK  $pkg" >> "$out"
        else
...
verify_cheshire_bender_common_cells_flist() {
...
verify_cheshire_verilator_stream_fifo_lint() {
    ...
    if ! command_exists verilator; then
        err "Verilator not on PATH (install via ./scripts/install_cheshire.sh or use --skip-verilator)"
        return 1
    fi
...
verify_cheshire_verilator_stream_fifo_binary() {
    ...
    if ! cheshire_cpp_compiler_ok; then
    ...
    verilator --binary -sv --no-timing -Mdir "$objdir" -f "$script_out" --top-module stream_fifo \
        -Wno-fatal -j "$CHESHIRE_VERILATOR_JOBS" \
        > "$log" 2>&1
    st=$?
    set -e
    if [[ "$st" -ne 0 ]]; then
...
verify_cheshire_verilator_full_sim() {
    ...
    if [[ ! -f "$CHESHIRE_TB_SV" ]]; then
        err "Missing harness $CHESHIRE_TB_SV"
        return 1
    fi
    if [[ ! -f "$script_out" ]]; then
    ...
    mkdir -p "$objdir"
    set +e
    verilator --binary -sv --timing -Mdir "$objdir" -f "$script_out" "$CHESHIRE_TB_SV" \
        --top-module tb_stream_fifo -Wno-fatal -j "$CHESHIRE_VERILATOR_JOBS" \
        > "$log" 2>&1
    st=$?
    set -e
    if [[ "$st" -ne 0 ]]; then
...
write_cheshire_verification_summary() {
    local vf="$VERIFICATION_DIR/verification_summary.txt"
    {
        echo "cheshire RTL verification summary (SCORE)"
        ...
        echo "  Full Cheshire SoC Verilator closure is not run by default in SCORE."
        echo "Logs: $VERIFICATION_DIR/"
    } > "$vf"
    ok "Wrote $vf"
}

run_cheshire_verification() {
    ...
    verify_cheshire_bender_lock_paths || return 1
    VERIFY_LOCK_RESULT=PASS

    verify_cheshire_bender_common_cells_flist || return 1
    VERIFY_FLIST_RESULT=PASS

    if [[ "$WITH_VERILATOR_STEPS" == true ]]; then
        verify_cheshire_verilator_stream_fifo_lint || return 1
        VERIFY_VLINT_RESULT=PASS

        verify_cheshire_verilator_stream_fifo_binary || return 1
        VERIFY_VBIN_RESULT=PASS

        if [[ "$WITH_FULL_SIM" == true ]]; then
            verify_cheshire_verilator_full_sim || return 1
            VERIFY_VSIM_RESULT=PASS
        else
            info "Verification: skipping full simulation (--no-full-sim or CHESHIRE_FULL_SIM=0)"
        fi
    else
...
CHESHIRE_COMMIT_ID=$(get_commit_id)
DATASET_DIR="$PROJECT_ROOT/datasets/cheshire/$CHESHIRE_COMMIT_ID"
...
if [[ "$VERIFY_ONLY" == true ]]; then
    ...
    if [[ "$VERIFY_ENABLED" != true ]]; then
        ...
        ok "generate_cheshire.sh completed (--verify-only)."
        exit 0
    fi
    if ! ( cd "$CHESHIRE_DIR" && bender path common_cells >/dev/null 2>&1 ); then
        err "Bender workspace not ready (e.g. bender path common_cells fails). Run checkout without --skip-checkout first."
        exit 1
    fi
    run_cheshire_verification || exit 1
    ok "generate_cheshire.sh completed (--verify-only)."
    exit 0
fi

pushd "$CHESHIRE_DIR" >/dev/null
...
if [[ "$VERIFY_ENABLED" == true ]]; then
    if [[ "$WITH_VERILATOR_STEPS" == true ]] && ! command_exists verilator; then
        err "verilator not on PATH. Run ./scripts/install_cheshire.sh or use --skip-verilator / --no-verify"
        exit 1
    fi
    if [[ "$WITH_VERILATOR_STEPS" == true ]] && ! cheshire_cpp_compiler_ok; then
...
SUMMARY="$DATASET_DIR/cheshire_summary.txt"
{
    ...
    if [[ "$VERIFY_ENABLED" == true ]]; then
        ...
        echo "  Verilator elaboration: ${VERIFY_VBIN_RESULT:-SKIPPED}"
        echo "  Verilator simulation: ${VERIFY_VSIM_RESULT:-SKIPPED}"
        echo "  Logs: ${VERIFICATION_DIR}/"
        echo ""
    fi
    echo "See https://github.com/pulp-platform/cheshire and tools/cheshire/README.md"
    echo "Bundle path: $BUNDLE_DIR"
    echo "Generation log: $SESSION_LOG"
} > "$SUMMARY"

ok "Wrote $SUMMARY"
ok "generate_cheshire.sh completed."
